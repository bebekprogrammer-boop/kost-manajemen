...
<style>
    ...
    .header-tenant-chip {
        ...
        border-radius: 99px;
        background: linear-gradient(135deg, rgba(22,163,74,0.1), rgba(29,78,216,0.08));
        font-size: 0.78rem;
        font-weight: 700;
        color: var(--g2);
    ...
    .header-tenant-avatar {
        ...
        border-radius: 6px;
        background: linear-gradient(135deg, rgba(22,163,74,0.2), rgba(29,78,216,0.15));
        display: flex; align-items: center; justify-content: center;
        font-size: 0.65rem;
        font-weight: 900;
        color: var(--g3);
    ...
    .form-input-locked {
        ...
        font-family: 'Roboto', sans-serif;
        color: #7a8a7a;
        background: linear-gradient(135deg, #f4faf4, #f0f4ff);
...
<div class="form-page">
    ...
    <div class="section-label">Edit Data Penghuni</div>

    <div class="form-card anim-in">
        <div class="card-header">
            <div class="card-header-icon">✏️</div>
            <div class="card-title">Perbarui Data</div>
            <div class="header-tenant-chip">
                <div class="header-tenant-avatar">{{ strtoupper(substr($tenant->name, 0, 1)) }}</div>
                {{ $tenant->name }}
            </div>
        </div>
            ...
            <form action="{{ route('admin.tenants.update', $tenant) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        <label>Kamar <span class="label-locked">🔒 tidak dapat diubah</span></label>
                        <div class="form-input-locked">
                            <span class="locked-icon">🏠</span>
                            {{ $tenant->room->room_number }} — {{ strtoupper($tenant->room->type) }}
                        </div>
                        <input type="hidden" name="room_id" value="{{ $tenant->room_id }}">
                    </div>

                    <div class="form-group">
                        <label for="rent_duration">Durasi Sewa Perpanjangan</label>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $tenant->name) }}" required class="form-input">
                        @error('name')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="phone" id="phone" value="{{ old('phone', $tenant->phone) }}" required class="form-input">
                        @error('phone')
                            <span class="field-error">⚠ {{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="check_in_date" id="check_in_date" value="{{ old('check_in_date', $tenant->check_in_date->format('Y-m-d')) }}" required class="form-input">
                        @error('check_in_date')
                            <span class="field-error">⚠ {{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email', $tenant->email) }}" placeholder="contoh@email.com" class="form-input">
                    </div>

                    <div class="form-group">
                        <label for="id_card_number">NIK <span class="label-optional">(nomor KTP)</span></label>
                        <input type="text" name="id_card_number" id="id_card_number" value="{{ old('id_card_number', $tenant->id_card_number) }}" placeholder="16 digit NIK" class="form-input">
                    </div>

                    <div class="form-group">
                        <label for="id_card_photo">Foto KTP Baru <span class="label-optional">(abaikan jika tidak ingin mengubah)</span></label>
                    ...
                    <div class="form-group full">
                        ...
                        <textarea name="notes" id="notes" class="form-textarea" placeholder="Catatan tambahan tentang penghuni...">{{ old('notes', $tenant->notes) }}</textarea>
                    </div>
