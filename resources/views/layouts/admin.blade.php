...
<body class="flex h-screen overflow-hidden font-sans bg-gray-100">

    <aside class="flex flex-col justify-between w-64 text-white bg-blue-900">
        <div>
            ...
            <nav class="flex flex-col mt-4 space-y-1">
                ...
                <a href="{{ route('admin.reports.index') }}" class="px-4 py-3 hover:bg-blue-800 {{ request()->routeIs('admin.reports.*') ? 'bg-blue-700' : '' }}">Laporan</a>

                @if(auth()->user()->role === 'super_admin')
                <a href="{{ route('admin.activity-logs.index') }}" class="px-4 py-3 hover:bg-blue-800 {{ request()->routeIs('admin.activity-logs.*') ? 'bg-blue-700' : '' }}">Log Aktivitas</a>
                @endif

                <a href="{{ route('admin.settings.index') }}" class="px-4 py-3 hover:bg-blue-800 {{ request()->routeIs('admin.settings.*') ? 'bg-blue-700' : '' }}">Pengaturan</a>
            </nav>
        </div>
        ...
        <div class="p-4 border-t border-blue-800">
            <div class="mb-2">
                ...
                <span class="px-2 py-1 text-xs bg-blue-700 rounded">{{ strtoupper(auth()->user()->role) }}</span>
            </div>
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="w-full text-sm text-left text-red-300 hover:text-red-100">
                    Logout
                </button>
            </form>
        </div>
